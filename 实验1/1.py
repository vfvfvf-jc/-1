...
# 将每个密文转换为整数
cipher_integers = [int(ct, 16) for ct in cipher_texts]
target_integer = int(target_text, 16)

# 计算每个密文与目标密文的异或结果
xor_results = [target_integer ^ ct for ct in cipher_integers]

# 处理异或结果，将其转换为可打印字符或问号
xor_strings = []
for xor_result in xor_results:
    xor_hex = format(xor_result, 'x')  # 转换为16进制字符串
    if len(xor_hex) % 2 != 0:
        xor_hex = '0' + xor_hex  # 确保每两个字符代表一个字节
    result = []
    for i in range(0, len(xor_hex), 2):
        byte = int(xor_hex[i:i + 2], 16)
        if 32 <= byte < 127:  # 只打印可显示字符
            result.append(chr(byte))
        else:
            result.append("?")  # 不可打印字符用"?"表示
    xor_strings.append(result)

# 将所有的xor结果对齐并打印出来
max_length = max(len(xor_string) for xor_string in xor_strings)
for xor_string in xor_strings:
    print("".join(xor_string).ljust(max_length))
